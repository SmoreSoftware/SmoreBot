...
client.on("message", message => {
  ...
  function restOfCode(prefix, adminrole, modlog, servername, serverid, serverowner, modrole) {
    ...
    if (message.content.startsWith(prefix + "ping")) {
      ...
      var getCurrentTime = require('performance-now');
      var startTime = getCurrentTime();
      message.channel.reply('Ping!').then(ping => {
        var endTime = getCurrentTime();
        ping.delete();

        var Result = new Discord.RichEmbed();

        var resTime = ((startTime - endTie) * 100);

        Result.setTitle('Bot Connection');

        var r = connections[2];
        if (resTime < 60) r = connections[1];
        if (resTime < 30) r = connections[0];

        Result.setColor(r.color);
        Result.addField(':timer: Ping: ', resTime + ' ms');
        Result.addField(':signal_strength: Connection: ', r.name);
